...
local cnm = string.find(LocalPlayer.LocaleId, "zh") ~= nil

local T = {
    Title = cnm and "钓它！" or "Fish It!",
    MainSection = cnm and "主功能" or "Main",
    MainTab = cnm and "主要功能" or "Main",
    AutoComplete = cnm and "自动完成钓鱼" or "Auto Complete Fishing",
    AutoSell = cnm and "自动出售" or "Auto Sell",
    PerfectFish = cnm and "完美钓鱼" or "Perfect Fishing",
    InfOxygen = cnm and "无限氧气" or "Infinite Oxygen",
    WaterWalk = cnm and "水上行走" or "Water Walk",
    AutoFish = cnm and "自动钓鱼" or "Enable Auto Fish",
    FakeLuck = cnm and "修改运气(假)" or "Change luck (false)",
    Author = cnm and "作者: Nova" or "Author: Nova",
    OpenUI = cnm and "打开UI" or "Open UI"
}
...
local Teleport = MainSection:Tab({ Title = T.TeleportTab, Icon = "Sword" })
...
Main:Toggle(
    {
        ...
        Image = "check",
        Callback = function(state)
            net:FindFirstChild("RF/UpdateAutoFishingState"):InvokeServer(state)
        end
    }
)
...
Main:Input(
    {
        ...
        Callback = function(value)
            for _,v in next,game:GetService("ReplicatedStorage").Items:GetChildren() do
                if v.Name:find("Rod") then
                    require(v).RollData.BaseLuck = value
                end
            end
        end
    }
